...
try {
  ...
  Write-Output "P1 cuenta: id=$($p1acc.data._id) totalAmount=$($p1acc.data.totalAmount) installments=$($p1acc.data.installments.Count)"
...
try {
  ...
  Write-Output "Cuota marcada. P1 virtual totalPaid=$($paid.data.totalPaid)"
...
try {
  ...
  Write-Output "Grupo after payment: virtual totalPaid=$($grpAcc2.data.totalPaid)"
  if ($grpAcc2.data.personTotals) {
    Write-Output "PersonTotals in response: totalPaid=$($grpAcc2.data.personTotals.totalPaid)"
  } else {
    Write-Output "PersonTotals NOT present in response"
  }
} catch { Write-Output "RE-FETCH ACCOUNTS ERROR: $_"; exit 1 }

Write-Output 'E2E script finished successfully'
